...
/**
 * UI-M6a: near-1:1 relocation of the old ProductsSection.tsx -- freestanding/
 * geometry-linked product placement, the Placed Products table (kept here
 * because the product-edge linker's From/To pickers need the same
 * `instances` list), suggestion acceptance, and product-to-product edge
 * linking. Furniture is excluded here (category filtered out below) since
 * it's placed exclusively through FurnitureCatalogue/DesignStageSection.
 */
export function ProductLinking({ designId: id, isDraft }: { designId: string; isDraft: boolean }) {
    ...
    <div>
      <div className="card">
        ...
        <div className="form-row">
          ...
          <button
            className="btn"
            disabled={!isDraft || !selectedSkuId || placeMutation.isPending}
            onClick={() =>
      ...
      {instances.length >= 2 && (
        <div className="card">
          ...
          <div className="form-row">
            ...
            <button
              className="btn"
              disabled={!isDraft || !fromInstanceId || !toInstanceId || linkInstancesMutation.isPending}
              onClick={() => {
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6, padding: "4px 0" }}>
              {suggestions.map((s) => (
                <div key={s.skuEdgeId} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13 }}>
                  ...
                  <button
                    className="btn"
                    disabled={!isDraft || acceptSuggestionMutation.isPending}
